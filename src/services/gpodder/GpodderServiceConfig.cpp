...
void GpodderServiceConfig::load()
{
    KConfigGroup config = KGlobal::config()->group( configSectionName() );
...
void GpodderServiceConfig::save()
{
    debug() << "save config";
...
void GpodderServiceConfig::askAboutMissingKWallet()
{
    if ( !m_askDiag )
    {
...
void GpodderServiceConfig::reset()
{
    debug() << "reset config";
    m_username = "";
    m_password = "";
...
void GpodderServiceConfig::textDialogYes() //SLOT
{
    DEBUG_BLOCK
...
void GpodderServiceConfig::textDialogNo() //SLOT
{
    DEBUG_BLOCK
